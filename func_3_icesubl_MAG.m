function [maxtime, Sublimation, esati, esatiM]=func_3_icesubl_MAG(RHi1,TK1,P1,z1,r1)
...
%Westbrook:
C(1)=0.25*dp(1);                          %Aggregates

%%
if  dp(1)<=450e-6  %&& dp(1)> 20e-6           %Mitchell: Aggregates of planar polycrystals in cirrus clouds
    alpha(1)=0.00739;
    beta(1)=2.45;
    gamma(1)=0.2285;
    sigma(1)=1.88;
elseif dp(1)> 450e-6 && dp(1)<=45000e-6  % dp(1)> 800e-6 (eigentl.)  %Mitchell: Aggregates of side planes, columns and bullets dmin wrong
    alpha(1)=0.0028;
    beta(1)=2.1;
    gamma(1)=0.2285;
    sigma(1)=1.88;
...
for t=2:1:1000000000                %dt in 0.01 sek 
    ...
    %C(t)=r1(t)*2/pi;   
    %Lamb&Verlinde:
    %phi(t)=0.05;    %Aspect ratio for plate=0.1-0.1, column=2-10, dendrites= 0.01-0.4 taken from Westbrook, 2008
    %epsilon=(1-phi(t)^2).^(1/2);
    %fc=epsilon/asin(epsilon);
    %C(t)=r1(t)*fc;
    
    %Westbrook:
    C(t)=0.25*dp(t);                          %Aggregates
    
    if  dp(t)<=450e-6  %&& dp(1)> 20e-6           %Mitchell: Aggregates of planar polycrystals in cirrus clouds
        alpha(t)=0.00739;
        beta(t)=2.45;
        gamma(t)=0.2285;
        sigma(t)=1.88;
    elseif dp(t)> 450e-6 && dp(t)<=45000e-6  % dp(1)> 800e-6 (eigentl.)  %Mitchell: Aggregates of side planes, columns and bullets dmin wrong
        alpha(t)=0.0028;
        beta(t)=2.1;
        gamma(t)=0.2285;
        sigma(t)=1.88;
