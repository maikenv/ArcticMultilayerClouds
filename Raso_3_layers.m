...
elseif NoRasoNum==0                                 %If a raso exists
    ...
    %[Raso10km, numcloud_i, Cloudixd] = func_1_layers(Raso,hmax,rhthres,minsub,minsuper); %std k=2 =_layers
    [Raso10km, numcloud_i, Cloudixd] = func_1_layers_1(Raso,hmax,rhthres,minsub,minsuper); %neu k=1 = _layers1
    ...
    if numcloud_i>=2                                   %If there are min 2 supersaturated layers 
        ...
        for k=1:layers                              %all subsaturated layers
            ...
            [maxtime(k), Sublimation(k), esati, esatiM]=func_3_icesubl(RHi1,TK1,P1,z1,r1);
