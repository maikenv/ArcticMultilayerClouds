...
str = 'a)';
dim = [.165 .09 .03 .07];
annotation('textbox',dim,'String',str,'FitBoxToText','on','BackgroundColor','none','LineStyle','none');
...
%%
%Save Plot: 
%a) if a filename (from Results_sort_plots) exists
%b) else in normal file

%try
%    mkdir('Plots',strxcat('7_Sectionlines_',filename));
%    saveas(gcf,strxcat('Plots/7_Sectionlines_',filename,'/',datestr(Cloudnet_short.N(1),'yyyy-mm-dd'),'.png')); 
%
%catch
    mkdir('Plots','7_Sectionlines'); 
    saveas(gcf,strxcat('Plots/7_Sectionlines/',datestr(Cloudnet_short.N(1),'yyyy-mm-dd'),'.png')); 
%end
...
clear h ax ax1 ax2 cloud cloudZ cloudZmean40 cloudZmean402 cloudZmeanhalf cloudZmeanhalf2 h h1 ...
    ...
    Nafter_more Nbefore_more str str_layer xt yt indexAtEnd_more indexAtStart_more
